...
boxplot(heart_data$age_years) #Here we can see that exists a clearly outlier in the age of 30. There are only 4 rows with that value, so they have to be removed.
heart_data<-filter(heart_data, age_years != 30)

boxplot(heart_data$age_years)
...
heart_data$bmi_class <- cut(heart_data$bmi, 
                            breaks = c(-Inf, 18.49, 24.9, 29.9, 34.9, 39.9, Inf),
                            labels = c("uw", "normal", "pre-ob", "ob 1", "ob 2", "ob 3"))
...
alco <- freq.table(heart_data, Alco, Cardio); alco

chisq.test(heart_data$Cardio, heart_data$Alco, correct=FALSE)
...
#Here it can be seen that drinking alcohol do not contribute to the develop of a cardiovascular disease. p-value = 0.05
...
chisq.test(heart_data$Cardio, heart_data$Active, correct=FALSE)
...
#Here we can see that being a non-active person contributes to the develop of a cardiovascular disease. p-value << 0.05

#Now if smokes contributes to the cardiovascular disease
smoke <- freq.table(heart_data, Smoke, Cardio)

chisq.test(heart_data$Cardio, heart_data$Smoke, correct=FALSE)
...
legend <- get_legend(smoke.plot +theme(legend.position = c(.45,.5), legend.direction="horizontal")) #Creating legend
...
chigender<-chisq.test(heart_data$Gender, heart_data$Cardio)
...
chisq.test(heart_data$Cholesterol, heart_data$Cardio, correct = FALSE)
...
#There is a relation between the cholesterol levels and the proportion of people with heart disease in population. p-value << 0.05

#Checking the same with the gluc

gluc<-freq.table(heart_data, Gluc, Cardio)

chisq.test(heart_data$Gluc, heart_data$Cardio, correct = FALSE)
...
#p-value << 0.05; high glucose levels are highly correlated to cardiovascular diseases.
...
#We can see here that older people has more risk to develop a cardiovascular disease than the young people
...
#As we can see here, exists a positive correlation between age and cardiovascular disease, and a linear model fits really well.
...
title_ages <- ggdraw() + draw_label("Age tendency in cardiovascular diseases", fontface='bold')          
...
#Defining function for eliminate outliers with IQRmethod
IQRmethod <- function(x,y){quartiles <- quantile(y, probs=c(.25, .75), na.rm = FALSE)
...
#Correlation between the BMI and cardiovascular diseases.
...
#The same that with the BMI, at higher MAP, higher percentages of people with cardiovascular disease
#In the group hyp2, it's where are more probability of having a cardiovascular disease 
...
map.tendency.plot<-ggplot(data = map.1, aes(x = `round(Map)`, y = freq, fill = Cardio)) +
  ...
  theme_half_open(12)+
  panel_border()+
  ggtitle("MAP tendency")+ #Add title
  theme(plot.title = element_text(hjust = 0.5))+ #Center the title
...
title_map <- ggdraw() + draw_label("MAP tendency in cardiovascular diseases", fontface='bold')          
